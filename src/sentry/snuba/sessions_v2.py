...
def _get_constrained_date_range(params, allow_minute_resolution=False):
    ...
    if interval > ONE_DAY:
        raise InvalidParams("interval has to be less than one day")

    if ONE_DAY % interval != 0:
        raise InvalidParams("interval not cleanly divisible")

    smallest_interval = ONE_MINUTE if allow_minute_resolution else ONE_HOUR

    if interval % smallest_interval != 0:
        raise InvalidParams("interval has to be a multiple of the minimum interval")
    ...
    if using_minute_resolution:
        if date_range.total_seconds() > 6 * ONE_HOUR:
            raise InvalidParams("one-minute resolution is restricted to 6h")
        if (datetime.now(tz=pytz.utc) - start).total_seconds() > 30 * ONE_DAY:
            raise InvalidParams("one-minute resolution is restricted to the last 30 days")
